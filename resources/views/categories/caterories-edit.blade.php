<x-guest-layout>
    <section class="">
        <form class="self-center" action="{{route('categories.update', $category->id)}}" method="POST">
            @csrf
            @method('PUT')
                            ...
                            @foreach($category->subCategories()->get() as $subcategory)
                                <tr class="subcategory-row">
                                    <td class="py-2 px-4 border-b">
                                        <input type="hidden" name="subcategories[{{ $loop->index }}][id]"
                                            value="{{ $subcategory->id }}">
                                        <x-text-input class="block w-full" type="text"
                                            name="subcategories[{{ $loop->index }}][name]"
    ...
    <script>
        let rowCount = {{ count($category->subCategories) }};
